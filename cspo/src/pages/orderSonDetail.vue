      ...
      <div class="stautsStyle">
        ...
        <el-card class="box-card" :body-style="payStyle" v-if="deatilSon.serviceType==='10'"  :key="deatilSon.serviceType">
          ...
          <div class="imgStyle">图片：
            <ul :id="'img'+deatilSon.customerImgs" @click="photoShow(deatilSon.customerImgs)">
              <li v-for="value in capitalize(deatilSon.customerImgs)" style="float:left">
                <img style="width:20px;height:20px;margin-right:5px;" :src="value" alt="">
              </li>
            </ul>
          </div>
          <div>订单金额：<span class="color3" v-if="deatilSon.totalPrice">￥{{deatilSon.totalPrice | fixedTwo}}</span></div>
          <div>结算信息：<span class="color3" v-if="deatilSon.totalPrice">商品售价：￥{{deatilSon.totalPrice | fixedTwo}}</span></div>
        </el-card>
        <!--退款信息-->
...
<script>
  ...
  import {cspOrderInfo, ERR_OK, reportDetail} from "@/api/api"
  ...
  export default {
    ...
    methods: {
      getDetail() {
        let data = {
          orderCode:this.checkedId
        };
        cspOrderInfo(data).then((res) => {
          if (res.code === ERR_OK) {
      ...
      _goDetail(val){
        ...
        this.$store.dispatch('getCustomerDetail',this.customerSonDetail);

    ...
    <div v-if="reportData">
      <div class="report-nav">
        <div class="nav-box">
          ...
          <a class="nav-item" :class="currentHash === 'summary' ? 'active' : ''" @click="_navClick('summary')">报告小结</a>
          <a class="nav-item" :class="currentHash === 'suggest' ? 'active' : ''" @click="_navClick('suggest')">总检建议</a>
        ...
        <el-card class="box-card" id="summary">
          ...
          <ul class="brief-summary">
            <li class="summary-list" v-for="(list,index) in reportData.GeneralAdvices">
              <p class="summary-text">小结名称：<strong>{{list.AdviceName}}</strong></p>
              <p class="summary-text">小结内容：{{list.AdviceDescription}}</p>
            </li>
          </ul>
        </el-card>

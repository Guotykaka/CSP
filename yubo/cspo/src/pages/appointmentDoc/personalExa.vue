    ...
    >
      ...
      <el-main>
        <el-table :data="tableData" border style="width: 100%" id="app">
          ...
          <el-table-column show-overflow-tooltip align="center" prop="idCard" label="身份证报告码"></el-table-column>
          <el-table-column show-overflow-tooltip align="center" prop="institutionName" label="体检机构"></el-table-column>
          <el-table-column show-overflow-tooltip align="center" prop="productName" label="体检套餐"></el-table-column>
          <el-table-column show-overflow-tooltip align="center" prop="appointmentTime" label="检查时间"></el-table-column>
          <el-table-column show-overflow-tooltip align="center" prop="appointmentStatus" label="预约状态">
          ...
          <el-table-column show-overflow-tooltip align="center" prop="appointmentStatus" label="到检状态">
            <template slot-scope="scope">
              <!-- <el-tag type="warning" v-if="scope.row.appointmentStatus===1">未到检</el-tag> -->
              <!-- <el-tag type="info" v-if="scope.row.appointmentStatus===2">待到检</el-tag> -->
          ...
          <el-table-column align="center" label="操作" width="280" fixed="right">
            <template slot-scope="scope">
              ...
              <el-button size="mini" v-if="scope.row.appointmentStatus===16" type="danger"  @click="ChangeStatus(false, scope.row)">预约拒绝
              </el-button>
              <el-button size="mini" v-if="scope.row.appointmentStatus===16" type="success"  @click="ChangeStatus(true, scope.row)"
              :disabled="checktime(scope.row.appointmentTime)">预约通过
              </el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      checktime(appointmentTime){//判断检查日期是否大于现在，否则disabled编辑按钮
        var date = new Date(appointmentTime + ':000').getTime();
        var myDate = new Date().getTime();
        return Boolean(myDate-date>0);
      },
        // 修改状态
      ChangeStatus(Status, row) {
          let text = '';
          let _appointmentStatus = null;
          if (Status) {
              text = '确定通过预约？';
              _appointmentStatus = 17;
          } else {
              text = '确定拒绝预约？';
              _appointmentStatus = 18;
          }
        this.$confirm(text, '提示', {
          ...
          .then(() => {
            let params = {
              examReserveId : row.examReserveId ,
              appointmentStatus : _appointmentStatus,
            };
            postPersonalExamStatus(params).then(response => {
      ...
      getList() {
        ...
        getPersonalExamList(params).then(response => {
          if (response.code === ERR_OK) {
            this.tableData = [];
      ...
      handleView(index, row) {
        ...
        getPersonalExamDetail(params).then((res) => {
          if (res.code === ERR_OK) {
            this.ViewItem = {};

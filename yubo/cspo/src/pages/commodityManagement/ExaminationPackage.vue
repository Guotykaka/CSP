  ...
  <div class="page-wrapper">
    ...
    <div class="page-container">
          ...
          <el-row>
            <el-form id="SearchBar" :model="searchParams" :inline="true" class="demo-form-inline">
              ...
              <el-form-item label="商品状态：">
                <el-select v-model="searchParams.productStatus" size="small" clearable placeholder="请选择商品状态">
            ...
            <el-table :data="tableData" border style="width: 100%">
              ...
              <el-table-column align="center" label="操作" width="300">
                <template slot-scope="scope">
                  ...
                  <el-button size="mini" type="primary" @click="handleEdit(scope.$index, scope.row)" :disabled="scope.row.productStatus=== 1">编辑</el-button>
      ...
      <div class="shop-edit" v-if="dialogEditVisible">
        <el-dialog title="编辑体检套餐商品" :visible.sync="dialogEditVisible" width=800px :before-close="handleCloseEidt">
          <el-form :model="editTable" :rules="rulesNew" ref="formEdit" :label-width="formLabelWidth">
            ...
            <el-form-item label="适用人群：" class='is-required'>
              <el-col :span="24">
                <el-radio-group v-model="editTable.forPeopleType">
                  <el-radio v-for="(item,index) in IntentedList" :key="index" :label="item.code">{{item.description}}</el-radio>
                </el-radio-group>
              </el-col>
            </el-form-item>
      ...
      <div class="shop-add" v-show="dialogAddVisible">
        <el-dialog title="新增体检套餐商品" :visible.sync="dialogAddVisible" width=800px :before-close="handleCloseAdd">
          <el-form :model="addTable" :rules="rulesNew" ref="formNew" :label-width="formLabelWidth">
            ...
            <el-form-item label="适用人群：" class='is-required'>
              <el-col :span="24">
                <el-radio-group v-model="addTable.forPeopleType">
                    <el-radio v-for="(item,index) in IntentedList" :key="index" :label="item.code">{{item.description}}</el-radio>
                </el-radio-group>
              </el-col>
            </el-form-item>
            ...
            <el-form-item label="商品描述：" class="is-required" style="margin-bottom:0">
            </el-form-item>
            <el-form-item label="" label-width="0px" prop="productDesc">
              <el-col :span="24">
            ...
            <!-- <el-form-item label="商品标签：" prop="commTagDetailEntities"> -->
              <el-col :span="16">
                <el-select filterable v-model="addTable.commTagDetailEntities" value-key="tagName" multiple
                :remote-method="remoteMethod" remote :loading="loading"
                           placeholder="请输入标签名搜索并进行选择">
                  <el-option :label="item.tagName" :value="item" v-for="(item,index) in RemoteOptions"
                             :key="item.tagName"></el-option>
    ...
    methods: {
      ...
      _doHandleEdit() {
        ...
        this.$refs['formEdit'].validate(valid => {
          if (valid) {
            let params = {
              forPeopleType: this.editTable.forPeopleType,
      ...
      SelectIntendList() {
        let params = {};
        SelectIntendList(params).then(response => {
          if (response.code === ERR_OK) {
            this.IntentedList = [];
            this.IntentedList = response.data;
          } else {
            IsAlert('获取供应商列表失败！', '错误', 'error');
          }
        }).catch(() => {

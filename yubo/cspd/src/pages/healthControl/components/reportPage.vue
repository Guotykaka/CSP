  ...
  <div class="report-page"
    ...
    <div class="report-main" v-if="reprotShow">
      ...
      <div class="report-nav">
        <el-row class="nav-text"><span>第{{selIndex+1}}份,共{{reportList.length}}份</span></el-row>
        <div class="card-box">
          <div v-if="showStatus" class="left-btn" @click="leftBtn">
            <i class="el-icon-arrow-left"></i>
          </div>
          <div class="item-box" ref="itemBox">
            <div class="item-list-box" ref="itemListBox">
              <div @click="selReport(index)" :class="{selItem:index===selIndex}" class="card-box-item" v-for="(item,index) in reportList" :key="index">
                <h3>体检日期:{{item.reportDateFormat}}</h3>
                <p>{{item.age}}岁</p>
                <p>{{item.checkUnitName}}</p>
                <p v-if="item.abandonCheckResults">共{{item.abandonCheckResults.length}}项异常</p>
                <p v-else>共0项异常</p>
              </div>
            </div>
          </div>
          <div v-if="showStatus" class="right-btn" @click="rightBtn"><i class="el-icon-arrow-right"></i></div>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    activated() {
      this.customerNameReport = this.$route.params.customerName;
      this.mobileReport = this.$route.params.mobile;
    ...
    methods: {
      ...
      getReportDetail() {
        ...
        this.reportDetail = {};
        let params = {
          customerName: this.customerNameReport,
          mobile: this.mobileReport
        };
        this.statusView = false;
        getUserReport(params).then((res) => {
          if (res.code === ERR_OK) {

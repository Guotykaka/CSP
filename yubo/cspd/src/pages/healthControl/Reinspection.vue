  ...
  <div class="recheck">
    ...
    <div class="recheck-table">
      <el-table
        ...
        <el-table-column show-overflow-tooltip prop="checkDay" align="center" label="过总检时间(天)" width="180"></el-table-column>
        <el-table-column fixed="right" width="280" align="center" label="操作">
          <template slot-scope="scope">
            <el-button v-if="scope.row.status===1&&messageBalance.haveAmount" type="warning" @click="delBtn(scope.row)" size="mini">结束计划</el-button>
      ...
      <el-dialog
        ...
        <el-form :model="recheckParams" :before-close="conputed_once">
          <el-form-item class="msgFlax" label="1." :label-width="formLabelWidth3">
            <el-radio v-for="(item,index) in msgList" :key="index" class="message" v-model="recheckParams.content" :label="item">{{item}}</el-radio>
          </el-form-item>
        </el-form>
        <div>
          <ul>

          ...
          <el-col :xs="12" :sm="10" :md="9" :lg="7"  v-for="(item,index) in serviceList" :key="index">
            <div class="service-item">
              ...
              <div class="btn-bar" v-if="item.whetherEdit">
                <input class="upline-btn" type="button" value="上线" @click="upLineFn(item)" v-if="item.productStatus===2">
                <input class="upline-btn" type="button" value="下线" @click="downLineFn(item)" v-if="item.productStatus===1">
...
<script>
import {ERR_OK,getTemplateService,updateProductStatus,saveService} from '@/api/api';
...
<style lang="less">
  .unit{padding-left: 10px;font-weight: bold}

  .suggest-price{padding-left: 30px;color:#409EFF;line-height: 40px;font-size: 16px}
  .rate-bar{font-size: 14px;color:#f9f9f9;line-height: 22px;margin-bottom: 5px}
  .template-card{
  ...
  .item-head{
    ...
    .price{float: right;line-height: 24px;font-size: 14px;color:#f6f6f6}
  }
  .time-title{
    font-size: 15px;
    color:#f6f6f6;
  ...
  .note{float: left;line-height: 40px;font-size: 15px;width: 85px;}
  .note-left {float: left;width: 490px;vertical-align: top;}

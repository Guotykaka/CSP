<template>
  <div class="recheckAdvice">
    <el-tabs v-model="activeBody" @tab-click="handleClickOne">
      <el-tab-pane label="复检提示" name="first">
        <div class="recheck-search">
          <el-form :inline="true" :model="searchParamsOne" class="demo-form-inline">
            <el-form-item label="计划名称">
              <el-input size="medium" v-model.trim="searchParamsOne.planName" placeholder="请输入计划名称"></el-input>
            </el-form-item>
            <el-form-item label="分组名称">
              <el-input size="medium" v-model.trim="searchParamsOne.groupName" placeholder="请输入分组名称"></el-input>
            </el-form-item>
            <el-form-item label="执行状态">
              <el-select size="medium" v-model="searchParamsOne.status" placeholder="请选择">
            ...
            <el-form-item>
              ...
              <el-button v-if="!messageBalance.haveAmount" size="medium" type="primary" @click="telPhone">联系客服
              </el-button>
            </el-form-item>
          </el-form>
          <el-row>
        ...
        <div class="recheck-table">
          <el-table
            ...
            <el-table-column fixed="right" width="280" align="center" label="操作">
              <template slot-scope="scope">
                <el-button v-if="scope.row.status===1&&messageBalance.haveAmount" type="warning"
        ...
        <div class="block">
          <el-pagination
            ...
            :current-page="searchParamsOne.currentPage"
            :page-sizes="[10, 20, 30, 40]"
            :page-size="searchParamsOne.pageSize"
        ...
        <el-dialog title="创建复检建议计划" :visible.sync="recheckStatus">
          <el-form :model="recheckParams" class="group-dialog">
            <el-form-item label="计划名称" :label-width="formLabelWidthOne">
              <el-input v-model="recheckParams.name"></el-input>
            </el-form-item>

            <el-form-item label="计划描述" :label-width="formLabelWidthOne">
              <el-input
            ...
            <el-form-item label="报告过总检后x天" :label-width="formLabelWidthOne">
              <el-input v-model="recheckParams.checkDay" type="number"></el-input>
            </el-form-item>

            <el-form-item label="执行日期" :label-width="formLabelWidthOne">
              <!--<el-date-picker-->
            ...
            <el-form-item label="执行时间" :label-width="formLabelWidthOne">
              <el-time-picker
            ...
            <el-form-item label="使用分组" class="group-select" :label-width="formLabelWidthOne">
              <el-select
                class="input-new-tag"
                @change="handleGroup"
            ...
            <el-form-item label="" :label-width="formLabelWidthOne">
              <el-tag
                :key="tag"
            ...
            <el-form-item label="干预内容" :label-width="formLabelWidthOne">
              <el-col :span="24">
                <el-input
                  type="textarea"
                  :rows="6"
      ...
      <el-tab-pane label="复检咨询" name="second">
        <div class="second-body">
          <div class="reins-advice">
            <div class="search-remark">
              <el-form :inline="true" :model="searchParams" class="demo-form-inline">
                <el-form-item label="">
                  <el-input v-model="searchParams.userName" placeholder="请输入客户姓名或备注" size="mini"></el-input>
                </el-form-item>
                <el-form-item>
                  <el-button type="primary" size="mini" @click="searchRemark">查 询</el-button>
                  <el-button size="mini" @click="clearRemark">清 空</el-button>
                </el-form-item>
              </el-form>
            </div>
            <div class="msg_main">
            ...
            <span slot="label" v-if="totalcountStart>0">
              ...
              >
                <div class="msgRight-header" v-if="chatCircleMsgShow">
                  <span class="fl">{{chatCircleMsg.customName}}</span>
                  <span class="fr">{{chatCircleMsg.createTime}}</span>
                </div>
                <div class="chat-box" v-cloak v-show="chatCircleMsgShow">
                  <div class="chat-main" ref="chatingTo">
                    <div class="user-detail" v-if="chatCircleMsg">
                    ...
                    <div class="chating-list">
                      ...
                      <ul class="chatlist-box">
                        <li v-for="(item,index) in chatList" :key="index">
                          ...
                          <div class="fl" v-if="messageShowLeft(item)&&item.flag===1"
                ...
                <div class="chat-input" v-show="customId||true">
                  ...
                  <div class="text-image">
                    ...
                    <textarea ref="textOn" v-show="!showLogo" @keyup.ctrl.enter="sendMsgSelec" placeholder="请输入内容"
                              v-model="showMsg"
                              class="text-area" @click="selmsgType"></textarea>
                  </div>
                  <div class="btn-main">
                    <div class="btn-text" v-if="!showLogo">ctrl+Enter&nbsp;可直接提交发送文字</div>
                    <el-button type="primary" size="mini" @click="sendMsgSelec"
                               @keyup.ctrl.enter="sendMsgSelec">发送
                    </el-button>
                  </div>
                </div>
              </div>
            ...
            <el-dialog :title="titleText" :visible.sync="adviceStatus" width="800px">
              ...
              <div v-if="adviceChatStatus">
                ...
                <el-form v-else :inline="true" :model="adviceParams" class="advice-style">
                  ...
                  <el-form-item>
                    <el-button type="primary" size="medium" @click="sureHandle">添加</el-button>
                  </el-form-item>
                </el-form>
              </div>
              <div slot="footer" class="dialog-footer">
            ...
            <el-dialog :show-close="false" :visible.sync="maskShow" width="60%" center class="detailRemark">
              <el-tabs v-model="detailName">
                <el-tab-pane label="详细" name="first">
                  <el-form v-if="detailText" :model="detailText" :label-width="remarkWidth"
                           style="width:80%;">
                    <el-form-item label="客户姓名：">
                      <span>{{detailText.name}}</span>
                    </el-form-item>
                    <el-form-item label="客户手机号：">
                      <span>{{detailText.phone}}</span>
                    </el-form-item>
                    <el-form-item label="性别：">
                      <span>{{detailText.gender | sexSelect}}</span>
                    </el-form-item>
                    <el-form-item label="出生日期：">
                      <span>{{detailText.birthday | birthdaySlice}}</span>
                    </el-form-item>
                    <el-form-item label="公司名称：">
                      <span>{{detailText.companyName}}</span>
                    </el-form-item>
                    <el-form-item label="分组名称：">
                      <span>{{detailText.groupNames}}</span>
                    </el-form-item>
                    <el-form-item label="备注：">
                      <el-input type="textarea" resize="none" v-model="updataeTradeDetail" :autosize="sizeText"
                                placeholder="输入备注信息，可以帮助您快速回顾历史服务内容"></el-input>
                    </el-form-item>
                    <el-form-item class="remark-btn">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getTableList() {
        let params = {
          currentPage: this.searchParamsOne.currentPage,
          groupName: this.searchParamsOne.groupName,
          pageSize: this.searchParamsOne.pageSize,
          planName: this.searchParamsOne.planName,
          institutionCode: this.institutionCode,
          status: this.searchParamsOne.status
        };
      ...
      /*查询列表*/
      onSearch() {
        this.searchParamsOne.currentPage = 1;
        this.searchParamsOne.pageSize = 10;
        this.getTableList()
      },
      /*清除查询参数*/
      clearParams() {
        this.searchParamsOne.currentPage = 1;
        this.searchParamsOne.groupName = '';
        this.searchParamsOne.pageSize = 10;
        this.searchParamsOne.planName = '';
        this.searchParamsOne.status = null;
      },
      /*选择每页条数*/
      handleSizeChangeOne(val) {
        this.searchParamsOne.pageSize = val;
        this.getTableList()
      },
      /*选择当前页*/
      handleCurrentChangeOne(val) {
        this.searchParamsOne.currentPage = val;
        this.getTableList()
      },
      /*新增复检*/
      addGroup() {
      ...
      getImgListEnd() {
        ...
        UsergetProcessedList(params).then((res) => {
          if (res.code === ERR_OK) {
      ...
      getImgListStart() {
        ...
        UsergetToBeTreatedList(params).then((res) => {
          if (res.code === ERR_OK) {
      ...
      sendMsgSelec() {
        ...
        haveRecheckSuggestion(paramss).then((res) => {
          if (res.code === ERR_OK) {
            if (res.data) {
              this.$alert('该用户的最新体检报告已写过体检建议');
            } else {
              this._upTextSend(params);
            }
          } else {
            this.$alert(res.msg, '提示')
          }
      ...
      /*详细信息展示*/
      detailShow(val) {
        this.tradeDetailId = val.customId;
        let params = {
          customerId: val.customId,
          institutionCode: this.userInfo.institutionCode
        };
        getCustomerRemark(params).then((res) => {
          if (res.code === ERR_OK) {
            this.maskShow = true;
      ...
      detailSave() {
        var params = {
          customerId: this.tradeDetailId,
          remark: this.updataeTradeDetail,
          institutionCode: this.userInfo.institutionCode
        };
        addCustomerRemark(params).then((res) => {
          if (res.code === ERR_OK) {
            this.detailText = res.data;
            this.maskShow = false;
          } else {
      ...
      getRemark() {
        ...
        let params = {
          searchStr: this.searchParams.userName,
          userId: this.searchParams.userId,
          currentPage: this.searchParams.currentPage,
          pageSize: this.searchParams.pageSize,
        };
        if (this.searchParams.userName === '' || this.searchParams.userName.length === 0) {
          this.$message.error('请输入搜索内容');
          return
        }
        medicalSearchedCustomerList(params).then((res) => {
          if (res.code === ERR_OK) {
            this.orderList = res.data;
            this.totalcount = res.data.totalCount;
            if (res.data.list.length > 0) {
      ...
      messageShowLeft(val) {
        if (val.type === '1' || val.type === '2' || val.type === '3' || val.type === '4' || val.type === '6' || val.type === '7') {
          return true;
        } else {
          return false;
        }
...
<style scoped lang="less">
  ...
  .detailRemark /deep/ .el-form-item {
    margin-bottom:0;
  }
  ...
  .reins-advice /deep/ .el-tabs__header {
    margin-bottom: 0;
  }

<template>
  <div class="imgText_consult imgtext-main" v-cloak>
                          ...
                          <div class="shop-box-card">
                            ...
                            <div class="shop-text">
                              ...
                              <p class="shop-t-bottom">
                                <span class="text-price">￥{{changeShopData(item.appendInfo,'productPrice')}}</span>
                                <del class="text-price origin-style">￥{{changeShopData(item.appendInfo,'originPrice')}}</del>
    ...
    <div id="shopBox" v-if="shopShowPush">
      ...
      <div class="shop-box">
        <div @click="pushShopMsg(item)" class="shop-box-card" v-for="(item,index) in shopList" :key="index">
          <div class="shop-img">
          ...
          <div class="shop-text">
            ...
            <p class="shop-t-bottom">
              <span class="text-price">￥{{item.productPrice}}</span>
              <del class="text-price origin-style">￥{{item.originPrice}}</del>
...
<script>
  ...
  import {
    ...
    getAppShopProductListFree,//查看所有配置的可推荐的供应链商品列表
    youPaiYunUpload,
    YOUPAIYUN_URL,
    getReportData,
    ...
    methods: {
      ...
      _upTextSend() {
        ...
        if (this.docubleClik) {
          ...
          freeSendToCustomer(params).then((res) => {
            if (res.code === ERR_OK) {
              this.showMsg = '';
              this.chatImg = null;
              this.showLogo = false;
      ...
      getImgListEnd() {
        ...
        getProcessedList(this.searchParams).then((res) => {
          if (res.code === ERR_OK) {
      ...
      /*获取当前健管师待处理列表*/
      getImgListStart() {
        getToBeTreatedList(this.searchParams).then((res) => {
          if (res.code === ERR_OK) {
      ...
      getShopList() {
        let params = {};
        getAppShopProductListFree(params)
          .then((res) => {
            if (res.code === ERR_OK) {
      ...
      messageShowRight(val){
        if(val.type==='1'||val.type==='2'||val.type==='3'||val.type==='9'){
          return true;
        }else{
          return false;
        }
...
<style scoped lang="less">
  ...
  .imgtext-main /deep/ .el-tabs__header {
    margin-bottom: 0;
  }
